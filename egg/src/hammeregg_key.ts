...
export async function importRSAPublicKey(pem: string): Promise<CryptoKey> {
    ...
    let contents = onelinePem.substring(
        PUBLIC_KEY_HEADER.length,
        onelinePem.length - PUBLIC_KEY_FOOTER.length
    )
    let der = string2Buffer(atob(contents))
    return crypto.subtle.importKey("spki", der, HAMMEREGG_RSA_PARAMS, false, [
        "wrapKey",
    ])
}
...
export async function importRSAPrivateKey(pem: string): Promise<CryptoKey> {
    ...
    let contents = onelinePem.substring(
        ...
        onelinePem.length - PRIVATE_KEY_FOOTER.length
    )
    let der = string2Buffer(atob(contents))
    return crypto.subtle.importKey("pkcs8", der, HAMMEREGG_RSA_PARAMS, false, [
        "unwrapKey",
    ])
}
